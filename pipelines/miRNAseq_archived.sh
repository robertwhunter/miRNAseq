...
(bowtie -n 1 -l 32 --norc --best --strata -m 1 --threads $set_cores \
  -x $set_genomes_dir/miRNA \
  $f \
  --un ${f}.miRNA.unaligned.fq \
  --sam ${f}.miRNA.sam) \
  2>${f}.miRNA.log

(bowtie -n 1 -l 32 --norc --best --strata -m 1 --threads $set_cores \
  -x $set_genomes_dir/stemloop \
  ${f}.miRNA.unaligned.fq \
  --un ${f%}.stemloop.unaligned.fq \
  --sam ${f}.stemloop.sam) \
  2>${f}.stemloop.log

done

# (bowtie -n 1 -l 32 --norc --best --strata -m 2 --threads $set_cores \
#  -x $set_genomes_dir/genome \
#  ${f}.stemloop.unaligned.fq \
#  --un ${f}.genome.unaligned.fq \
#  --sam ${f}.genome.sam) \
#  2>${f}.genome.log
# 
# done

mkdir map; mv trim/*miRNA* trim/*stemloop* map
# mkdir map; mv trim/*miRNA* trim/*stemloop* trim/*genome* map
...
parallel "umi_tools count \
  --method=unique --per-contig -I \
  {} \
  -L {.}.counts.log \
  -S {.}.counts.txt" ::: map/*.bam

mkdir counts; mv map/*counts* counts
mv trim/lib_size.txt counts


# de-duplicate - is this redundant as appears to give same results as above

# parallel "umi_tools dedup \
#  --method=unique -I \
#  {} \
#  -S {.}.deduplicated.bam \
#  -L {.}.deduplicated.log" ::: map/*.miRNA.bam
#
# parallel "samtools index {}" ::: map/*.deduplicated.bam
# parallel "samtools idxstats {} | cut -f1,3 - | sed "1s/^/miRNA\t${i}-miRNAcount\n/" - > {.}.txt" ::: map/*.deduplicated.bam
# 
# mv map/*deduplicated.txt counts
